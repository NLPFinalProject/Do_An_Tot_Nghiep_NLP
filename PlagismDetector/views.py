...
def documentimport(request):
    ...
    cursor.execute(
        "SELECT DataDocumentSentence FROM polls_datadocumentcontent WHERE DataDocumentNo_id='1';"
    )
    ...
    fName, lstSentence, lstLength = p.preprocess(
        settings.MEDIA_ROOT
        + "\\DocumentFile\\"
        + dataFromFile.DataDocumentName
        + "."
        + dataFromFile.DataDocumentType
    )
    # print(lstSentence)
    lst2 = lstSentence
    ...
    if request.method == "POST":
        ...
        if form1.is_valid():
            ...
            for i in range(length):
                c = data.datadocumentcontentt_set.create(
