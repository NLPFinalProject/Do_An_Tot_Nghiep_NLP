...
class SiameseBiLSTM:
    ...
    def train_model(
        self, sentences_pair, is_similar, embedding_meta_data, model_save_directory="./"
    ):
        """
        ...
        tokenizer, embedding_matrix = (
            embedding_meta_data["tokenizer"],
            embedding_meta_data["embedding_matrix"],
        )
        ...
        leaks_dense = Dense(
            int(self.number_dense_units / 2), activation=self.activation_function
        )(leaks_input)
        ...
        merged = Dense(self.number_dense_units, activation=self.activation_function)(
            merged
        )
        merged = BatchNormalization()(merged)
        merged = Dropout(self.rate_drop_dense)(merged)
        ...
        model = Model(
            inputs=[sequence_1_input, sequence_2_input, leaks_input], outputs=preds
        )
        ...
        STAMP = "lstm_%d_%d_%.2f_%.2f" % (
            self.number_lstm_units,
            self.number_dense_units,
            self.rate_drop_lstm,
            self.rate_drop_dense,
        )

        checkpoint_dir = (
            model_save_directory + "checkpoints/" + str(int(time.time())) + "/"
        )
    ...
    def update_model(
        self, saved_model_path, new_sentences_pair, is_similar, embedding_meta_data
    ):
        """
        ...
        new_model_checkpoint_path = (
            saved_model_path.split("/")[:-2] + str(int(time.time())) + "/"
        )

        new_model_path = new_model_checkpoint_path + model_file_name
        model_checkpoint = ModelCheckpoint(
            new_model_checkpoint_path + model_file_name,
        ...
        tensorboard = TensorBoard(
            log_dir=new_model_checkpoint_path + "logs/{}".format(time.time())
        )
