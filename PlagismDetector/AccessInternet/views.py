...
from ScrapeSearchEngine.ScrapeSearchEngine import \
    Google, Duckduckgo, Givewater, Bing, Yahoo
...
def DownloadDocument(url):
    if (IsDownloadable(url)):
        ...
        f = open(settings.MEDIA_ROOT +
                 '\\DocumentFile\\file_downloaded\\' + name, 'wb')
        f.write(r.content)
        return settings.MEDIA_ROOT + '\\DocumentFile\\file_downloaded\\' + name


# crawl text from html website then preprocess
# and give output: list sentence after preprocessing.
def CrawlWeb(url):
    try:
