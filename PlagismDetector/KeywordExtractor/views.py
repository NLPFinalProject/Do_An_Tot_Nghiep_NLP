...
def crawlWeb(url):
    try:
        ...
        temp = text.split("\n")
        listSentence = p.ListSentence(temp)
        temp1 = p.SplitSentList(listSentence, 450)
...
def checkWordInSen(word, tokenize):
    count = 0
    for s in tokenize:
        if word in s:
            count += 1
...
def IDF(words, tokenize_lower):
    ...
    for key, val in words.items():
        idf[key] = math.log(
            len(tokenize_lower) / (1 + checkWordInSen(key, tokenize_lower))
        )
